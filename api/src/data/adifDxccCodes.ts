// Maps ADIF's standardized numeric DXCC Entity Code (adif.org.uk ADIF 3.1.5
// spec, "DXCC Entity Code Enumeration") to this site's own canonical entity
// names (dxccEntities.ts, drawn from cty.dat) -- needed because Club Log's
// most-wanted API (clublogMostWanted.ts) ranks by this numeric code, not by
// name, and the two naming conventions differ (ADIF: "UNITED STATES OF
// AMERICA", all-caps, DXCC-program-official phrasing; cty.dat: "United
// States", title case, sometimes different wording entirely e.g. "IS." vs
// no suffix at all).
//
// Deleted DXCC entities are excluded entirely (can't be "most wanted" if you
// can no longer work it). Entity code 390 ("Turkey") is also deliberately
// excluded -- ADIF unified Asiatic/European Turkey into one code after the
// 2003 DXCC merger, but this site's cty.dat-derived list still carries the
// pre-merger split (two separate entities, two different CQ zones), so a
// single ADIF code can't be mapped to one without arbitrarily guessing which
// half. A most-wanted rank for code 390 is simply dropped rather than risk
// mislabeling one of the two.
//
// Hand-verified against dxccEntities.ts (2026-08-11) -- every value here is
// confirmed to exist verbatim in DXCC_ENTITIES.
export const ADIF_DXCC_CODE_TO_ENTITY: Record<number, string> = {
  1: 'Canada',
  3: 'Afghanistan',
